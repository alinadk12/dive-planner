...
class FileHelper extends YiiFileHelper
{
     ...
     * @return string|mixed
     *
     * @throws Exception Если файл не обычный файл или нет доступа на чтение.
     */
    public static function fileGetContents($path, $useIncludePath = null, $offset = null)
    {
        if (! static::isFile($path) || ! static::isReadable($path)) {
            throw new Exception('The file is not a regular file or not readable');
        }
     ...
     *
     * @return true
     * @throws Exception генерируется в случае проблем с удалением.
     */
    public static function remove($path, $strictly = true)
    {
        ...
        throw new Exception('The file or directory in the specified path does not exist');
    }
     ...
     *
     * @return true
     * @throws Exception генерируется в случае проблем с чтением файла или директории.
     */
    protected static function deleteDirectory($path)
    {
        if (! static::isDirectory($path)) {
            throw new Exception('The directory in the specified path does not exist');
        }
        ...
        try {
            ...
            throw new Exception('Error while deleting directory');
        }
        return true;
    }
    ...
    {
        if (! static::isFile($path)) {
            throw new Exception('The file in the specified path does not exist');
        }
        if (! static::isWritable($path)) {
            throw new Exception('The path is not writable');
        }
     ...
     *
     * @return integer
     * @throws Exception генерируется в случае проблем с чтением файла.
     */
     ...
     * @return true
     *
     * @throws Exception генерируется в случае проблем с созданием директории.
     */
     ...
     *
     * @return true
     * @throws Exception генерируется в случае проблем с переименовыванием файла или директории.
     */
    public static function rename($oldName, $newName, $path = null)
    {
        ...
        if (! rename($oldName, $newName)) {
            throw new Exception('Error while rename file or directory');
        }
        return true;
    }
     ...
     * @return void
     *
     * @throws Exception Исключение генерируется в случае, если файл не был закрыт.
     */
    public static function fileWrite($path, $content, $lock = true, $mode = '')
    {
        ...
        if (! self::isWritable($fileDir)) {
            throw new Exception(sprintf('Directory "%s" is not writable', $fileDir));
        }
        ...
        if ($lock && ! flock($fp, LOCK_EX)) {
            throw new Exception(sprintf('Can not lock file "%s"', $path));
        }
        if (false === fwrite($fp, $content)) {
            throw new Exception(sprintf('Can not write file "%s"', $path));
        }
        if ($lock && ! flock($fp, LOCK_UN)) {
            throw new Exception(sprintf('Can not unlock file "%s"', $path));
        }
        if (false === fclose($fp)) {
            throw new Exception(sprintf('Can not close file "%s"', $path));
        }
    }
    ...
    public static function getSize($dir)
    {
        ...
        if (false === $size) {
            throw new Exception('Error while reading file or directory');
        }
        return $size;
    }
    ...
    protected static function fileSize($path)
    {
        ...
        try {
            $size = filesize($path);
            if (false === $size) {
                throw new Exception('Error while getting size');
            }
        } catch (Exception $e) {
            throw new Exception('Error while getting size');
        }
    ...
    public static function scanDir($path)
    {
        ...
        try {
            ...
            if (false === $directoryList) {
                throw new Exception('Error while scanning dir');
            }
    ...
    public static function fileModifiedTime($path)
    {
        ...
        try {
            $time = filemtime($path);
            if (false === $time) {
                throw new Exception('Error while scanning file');
            }
            return $time;
        } catch (Exception $e) {
            throw new Exception('Error while scanning file');
     ...
     * @return string
     *
     * @throws Exception Генерирует в случае отсутствия прав на запись.
     */
